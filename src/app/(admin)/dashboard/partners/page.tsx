...
function PremiumPartners() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, deletedId) => {
      queryClient.setQueryData(["partners"], (old: Partner[] = []) => old.filter((partner) => partner.id !== deletedId))
    },
  })
    ...
    <div className="sm:max-w-6xl mx-auto w-full space-y-8 p-4">
      <div className="space-y-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-balance">Premium Partners</h1>
            <p className="text-muted-foreground text-pretty">Manage and showcase your premium partner logos</p>
          </div>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-12 text-center space-y-3">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">Upload your first partner logo to get started</p>
              </div>
            </CardContent>
          </Card>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
            {partners.map((partner) => (
              <Card key={partner.id} className="group hover:shadow-md transition-all duration-200 hover:scale-[1.02] relative">
                <Button
                  onClick={() => deleteMutation.mutate(partner.id)}
                  size="sm"
                  variant="destructive"
                  className="absolute rounded-full top-2 right-2 z-10 p-2 h-auto opacity-0 group-hover:opacity-100 transition-opacity"
