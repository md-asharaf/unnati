  ...
  return (
    <div className="flex flex-col gap-10">
          ...
          <Card key={course.id} className="group overflow-hidden">
            <CardHeader className="p-0">
              <div className="relative aspect-[16/9] overflow-hidden">
                ...
                <span className="absolute top-2 left-2 rounded-full bg-background/80 backdrop-blur px-3 py-1 text-[11px] font-medium tracking-wide line-clamp-1">{course.subtitle}</span>
              </div>
            </CardHeader>
            <CardContent className="pt-4 flex flex-col gap-3">
              <Stars value={Math.random() * 5} />
              <CardTitle className="text-base leading-snug line-clamp-2 group-hover:text-secondary transition-colors">{course.title}</CardTitle>
              {course.description && (
      ...
      {!hasNextPage && !isFetchingNextPage && (
        <div className="text-center text-xs text-muted-foreground">No more courses</div>
      )}
    </div>
  );
}
